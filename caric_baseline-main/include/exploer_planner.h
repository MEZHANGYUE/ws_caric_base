...
{
public:
    Exploer(ros::NodeHandlePtr &nh_ptr_)
    : nh_ptr(nh_ptr_)
    {

        TimerProbeNbr = nh_ptr->createTimer(ros::Duration(1.0 / 10.0), &Exploer::TimerProbeNbrCB, this);
        TimerPlan     = nh_ptr->createTimer(ros::Duration(1.0 / 2.0),  &Exploer::TimerPlanCB,     this);
        TimerCmdOut   = nh_ptr->createTimer(ros::Duration(1.0 / 10.0), &Exploer::TimerCmdOutCB,   this);
        TimerViz      = nh_ptr->createTimer(ros::Duration(1.0 / 1.0),  &Exploer::TimerVizCB,      this);
        boxes_sub_ = nh_ptr->subscribe("/boxes_path" , 10, &Exploer::BoxesCallback, this);   //订阅边界框的路径点，用于探索者
        task_sub_ = nh_ptr->subscribe("/task_assign" + nh_ptr->getNamespace(), 10, &Exploer::TaskCallback, this);
        
        com_sub_  = nh_ptr->subscribe("/broadcast" + nh_ptr->getNamespace(), 10, &Exploer::ComCallback, this);
        ...
        odom_sub_        = nh_ptr->subscribe("/ground_truth/odometry", 10, &Exploer::OdomCallback, this);
        gimbal_sub_      = nh_ptr->subscribe("/firefly/gimbal", 10, &Exploer::GimbalCallback, this);
        ...
        sync_->registerCallback(boost::bind(&Exploer::MapCallback, this, _1, _2, _3));
                     ...
                     const nav_msgs::OdometryConstPtr &msg)
    {
        ...
        // if ((last_time - start_time).toSec()>1)
        // {
        //     last_time = start_time ;
        //     cloud_all+=cloud_;
        //     // pcl::io::savePCDFile("/home/yue/ws_caric/src/record/recorded_point_cloud.pcd", cloud_all);
        //     pcl::io::savePCDFile("/home/yue/ws_caric/src/vision_create/model/recorded_point_cloud.pcd", cloud_all);
        //     ROS_INFO("Point cloud saved to recorded_point_cloud.pcd");
        // }
    }
...
private:
    ...
    vector<vector<Eigen::Vector3d>> waypoint_set;
    visualization_msgs::MarkerArray last_msg;
    int waynumber = 0 , pointnumber = 0 ;
    Eigen::Vector3d tar_point;
    std::vector<std::vector<Eigen::Vector3d>> visual_to_vector(visualization_msgs::MarkerArray msg )
    {
        std::vector<std::vector<Eigen::Vector3d>> waypoint_set_;
        std::vector<Eigen::Vector3d> way;
        ...
        for ( int i=0 ; i<msg.markers.size() ; i++)
        {
            ...
            waypoint_set_.push_back(way);
            way.clear();
        }
        return waypoint_set_;
    }
    void BoxesCallback (const visualization_msgs::MarkerArrayConstPtr &msg)  //订阅探索者沿着边界框的路径点，可视化信息转化成向量信息
    {
        if(last_msg == *msg) return;
        else last_msg=*msg;
        waypoint_set = visual_to_vector(*msg);
    }
    ...
    void TimerPlanCB(const ros::TimerEvent &)
    {
        ...
        tar_point = waypoint_set[waynumber][pointnumber];
        if((my_position - tar_point).norm() <2 )
        {
            pointnumber ++ ;
            if (pointnumber>=waypoint_set[waynumber].size())
            {
                pointnumber = 0;
                waynumber ++ ;
            }
